...
<p>はい - Webhookを使えば、チームのバックエンドやCRM、自動化ツール(Zapier、n8n、Make)へNFCやQRのスキャンを直接送信できます。NFC.coolのアカウントもアップロードも不要で、データが当社のサーバーを通ることはありません。大規模なNFCタグ・キャンペーンやカスタムブランディングのご相談は<a href="mailto:info@nfc.cool?subject=NFC.cool%20Support">メールでお問い合わせください</a>。</p>
</details>
...
<p>返金はAppleまたはGoogleが直接対応しており、当社では処理しません。iOS: 設定 → App Store → 名前をタップ → 購入履歴。Android: Google Play → アカウント → 注文履歴。</p>
</details>
