...
WebhookはJSONの`POST`を指定URLへ送るだけ - HTTPを話すものなら何でも繋げられます。
...
Webhookは **HTTP Basic認証のみ** をサポートします。**設定 → Webhook** で、ユーザー名とパスワードをiOSキーチェーンに保存できます(任意)。これらの値を使い、サーバーからの`401 / WWW-Authenticate: Basic`チャレンジに応答します。
