---
id: nfc-blog-028
title: "Je n'arrive pas à me souvenir des gens que je rencontre. Alors je l'ai intégré à l'app de carte de visite."
date: 2025-01-23
tags: ["business-cards", "networking", "iphone"]
summary: "Après suffisamment de conférences et d'événements de réseautage, j'ai réalisé que les cartes de visite numériques résolvaient le mauvais problème. Elles sauvaient des arbres, mais pas le contexte. Alors j'ai ajouté une couche Smart Context à NFC.cool Business Card - où vous vous êtes rencontrés, sur quoi la personne travaille, ce qu'il y a à relancer."
metaTitle: "Smart Context: la mise à niveau mémoire des cartes de visite numériques"
metaDescription: "Les cartes de visite numériques règlent le problème du papier. Elles ne règlent pas celui d'oublier les gens. La couche Smart Context de NFC.cool Business Card capture le où, le quand et le pourquoi - synchronisé avec votre calendrier."
ogTitle: "Je n'arrive pas à me souvenir des gens que je rencontre. Alors je l'ai intégré à l'app de carte de visite."
ogDescription: "Pourquoi les cartes de visite numériques avaient besoin de plus que des coordonnées - et la couche Smart Context qui règle le problème."
image: "/assets/images/Blog/smart-context-remember-everyone.webp"
---
Après des années à développer des logiciels de carte de visite numérique, il y avait un problème qui n'arrêtait pas de me tracasser: nous avions résolu la mauvaise moitié.

Le problème de la carte papier est réel - les cartes se périment, remplissent votre portefeuille, se perdent, ne peuvent pas être mises à jour. Les cartes numériques ont réglé ça. Mais elles n'ont pas réglé le vrai problème du réseautage, qui est bien plus simple:

> Je rencontre 50 personnes à une conférence, j'échange mes coordonnées avec 20, et trois semaines plus tard je ne me souviens pas d'une seule conversation.

Les coordonnées dans votre téléphone ne servent à rien si vous ne vous souvenez pas pourquoi cette personne est dans votre répertoire.

---

## Le contexte, c'est la pièce manquante

Alors j'ai ajouté à NFC.cool Business Card ce que j'appelle depuis la « mise à niveau mémoire ». Juste après la connexion - via le geste NFC, l'App Clip, ou le QR sur écran verrouillé du mode Conférence - on vous invite à noter le contexte:

- **Où et quand vous vous êtes rencontrés.** Rempli automatiquement avec la date et le lieu, modifiable.
- **Sur quoi la personne travaille.** Une courte note sur son projet, son entreprise ou son domaine.
- **Les temps forts de la conversation.** Les une ou deux choses dont vous avez vraiment parlé et dont vous voudriez vous souvenir.
- **Les relances à prévoir.** « Elle va me mettre en relation avec son VC. » « Envoyer le deck lundi. »

Ce dernier point se synchronise avec votre calendrier et vos rappels, parce qu'on est tous mauvais pour donner suite et qu'on a tous besoin d'un petit coup de pouce.

---

## Pourquoi ça fait partie de l'échange, et non après

L'astuce, c'est que l'invite apparaît juste après l'enregistrement du contact - pendant que la conversation est encore fraîche dans votre tête. Cinq minutes plus tard, vous êtes passé à la personne suivante. Trois jours plus tard, vous ne savez plus si le fondateur d'IA était celui du concours de pitch d'Austin ou du hackathon de Berlin.

Saisir le contexte dans le même parcours que l'échange de contact, ça veut dire que les informations sont réellement notées. L'alternative - ajouter le contexte manuellement la semaine suivante, de mémoire - n'arrive jamais.

---

## Ce que ça a changé pour moi

Pendant les tests bêta lors de quelques événements, l'expérience est passée de « j'ai maintenant ces cartes de visite dans mon téléphone » à « j'ai un graphe interrogeable de personnes, de ce qu'elles font et de ce que je leur dois ».

J'ouvre l'onglet Networking dans NFC.cool Business Card et je vois: qui j'ai rencontré et où, ce dont on a parlé, ce que j'ai dit que je relancerais, ce qui reste ouvert. Après avoir revu quelqu'un, je mets à jour la fiche - nouvelle conversation, nouveau contexte. La carte devient un journal vivant de la relation, et non un instantané de coordonnées.

---

## Fonctionne sur toute la chaîne

La couche Smart Context fonctionne quelle que soit la façon dont le contact est arrivé dans votre répertoire:

- **Geste NFC.** Le parcours standard - vous approchez leur carte, enregistrez le contact, saisissez le contexte.
- **App Clip.** Les destinataires iOS voient la surcouche App Clip, enregistrent le contact et reçoivent la même invite de contexte.
- **Mode Conférence (QR sur écran verrouillé).** Affichez votre QR sur écran verrouillé pour un échange rapide dans les environnements bruyants; la même invite de contexte se déclenche dès qu'ils enregistrent.
- **Navigateur Android.** Les destinataires Android ouvrent la version page web, enregistrent le contact, et peuvent ensuite saisir le contexte dans l'app NFC.cool Business Card.

L'app gère jusqu'à 100 cartes différentes (différents rôles, différents événements, différentes versions de vous) et les données Smart Context restent séparées par carte - ainsi, un contact rencontré en tant que « consultant en design au meetup de Berlin » est une fiche différente de la même personne rencontrée en tant que « cofondateur au demo day de YC ».

---

## Pourquoi c'est important maintenant

Si ça n'existait pas il y a cinq ans, c'est que le goulet d'étranglement n'était pas la technique - c'était la friction. Saisir le contexte imposait de sortir une app de notes séparée, de taper pendant que l'autre personne regardait, puis d'associer tant bien que mal ces notes au contact plus tard. La plupart des gens abandonnaient.

Avec NFC.cool Business Card, la saisie se fait d'un seul geste, dans la continuité de l'échange de contact. C'est toute la différence entre « je devrais m'en souvenir » et « c'est désormais mémorisé ».

Dans un monde où l'on échange des contacts plus vite que jamais, la donnée qui compte n'est pas qui vous connaissez - c'est pourquoi vous les connaissez.

[Téléchargez NFC.cool Business Card pour iPhone](https://apps.apple.com/app/apple-store/id6502926572?pt=106913804&ct=blog-smart-context-remember-everyone-fr&mt=8). Les utilisateurs Android retrouvent la même carte de visite et les mêmes fonctions Smart Context dans [NFC.cool Business Card pour Android](https://play.google.com/store/apps/details?id=cool.nfc.businesscard&referrer=utm_source%3Dnfc.cool%26utm_medium%3Dblog%26utm_campaign%3Dblog-smart-context-remember-everyone-fr).
