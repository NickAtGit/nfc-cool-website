...
| `/apps/{slug}/` | `Content/Pages/Apps*.md` per-app promo subpages (slugs `moments`, `qr-code-scanner`, `3d-scanner`, `flight-tracker`, `pokemon-quiz`, `ebike-monitor`, `mondego-move`) - EN-only via `enOnly` in `i18n.yaml`, localized URLs get fallback redirects; exception: `moments` (`AppsMoments{.<lang>}.md`) is fully localized; store URLs + JSON-LD ratings live in `Sources/Site/Helpers/AppCatalog.swift` |
